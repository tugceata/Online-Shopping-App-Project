...
public class AddBookPage extends JFrame { 
    ...
    public AddBookPage() {
        ...
        addBookPageBtnActionListener back = new addBookPageBtnActionListener();
        backBtn.addActionListener(back);
        bottomPanel.add(backBtn);

        addProductBtn = new JButton("Ürünü Kaydet");
        addBookPageBtnActionListener add = new addBookPageBtnActionListener();
    ...
    private class addBookPageBtnActionListener implements ActionListener {
        @Override
        public void actionPerformed(ActionEvent evt) {
            if (evt.getSource() == addProductBtn) {
                ...
                try (BufferedReader br = new BufferedReader(new FileReader("src/bookproducts.txt"))) {
                    String line;
                ...
                if (!found) {
                    try (FileWriter filewriter = new FileWriter("src/bookproducts.txt", true)) {
                        maxID++; // Increment maxID for new entry
                        String temp = ("Kitap," + maxID + "," + productNameTf.getText().trim() + ","
                                + productPriceTf.getText().trim() + "," + productStockTf.getText().trim() + "\r\n");
                        filewriter.write(temp);
                        JOptionPane.showMessageDialog(null, "Yeni ürün kaydedildi.");
                        dispose();
                        new AdminBookPage().setVisible(true);
                    } catch (IOException e) {
