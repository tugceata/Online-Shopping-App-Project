...
public class AddMarketPage extends JFrame { 
    ...
    public AddMarketPage() {
        ...
        addMarketPageBtnActionListener back = new addMarketPageBtnActionListener();
        backBtn.addActionListener(back);
        bottomPanel.add(backBtn);

        addProductBtn = new JButton("Ürünü Kaydet");
        addMarketPageBtnActionListener add = new addMarketPageBtnActionListener();
    ...
    private class addMarketPageBtnActionListener implements ActionListener {
        @Override
        public void actionPerformed(ActionEvent evt) {
            if (evt.getSource() == addProductBtn) {
                ...
                try (BufferedReader br = new BufferedReader(new FileReader("src/marketproducts.txt"))) {
                    String line;
                ...
                if (!found) {
                    try (FileWriter filewriter = new FileWriter("src/marketproducts.txt", true)) {
                        maxID++; 
                        String temp = ("Market," + maxID + "," + productNameTf.getText().trim() + ","
                                + productPriceTf.getText().trim() + "," + productStockTf.getText().trim() + "\r\n");
                        filewriter.write(temp);
                        JOptionPane.showMessageDialog(null, "Yeni ürün kaydedildi.");
                        dispose();
                        new AdminMarketPage().setVisible(true);
                    } catch (IOException e) {
