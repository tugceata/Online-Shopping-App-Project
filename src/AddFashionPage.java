...
public class AddFashionPage extends JFrame { 
    ...
    public AddFashionPage() {
        ...
        bottomPanel.setBackground(backgroundcolor.getBackgroundColor()); // Commented out as backgroundcolor is not defined
        bottomPanelFl = new FlowLayout(FlowLayout.CENTER);
        bottomPanel.setLayout(bottomPanelFl);

        backBtn = new JButton("Geri Dön");
        addFashionPageBtnActionListener back = new addFashionPageBtnActionListener();
        backBtn.addActionListener(back);
        bottomPanel.add(backBtn);

        addProductBtn = new JButton("Ürünü Kaydet");
        addFashionPageBtnActionListener add = new addFashionPageBtnActionListener();
    ...
    private class addFashionPageBtnActionListener implements ActionListener {
        @Override
        public void actionPerformed(ActionEvent evt) {
            if (evt.getSource() == addProductBtn) {
                ...
                try (BufferedReader br = new BufferedReader(new FileReader("src/fashionproducts.txt"))) {
                    String line;
                ...
                if (!found) {
                    try (FileWriter filewriter = new FileWriter("src/fashionproducts.txt", true)) {
                        maxID++; // Increment maxID for new entry
                        String temp = ("Moda," + maxID + "," + productNameTf.getText().trim() + ","
                                + productPriceTf.getText().trim() + "," + productStockTf.getText().trim() + "\r\n");
                        filewriter.write(temp);
                        JOptionPane.showMessageDialog(null, "Yeni ürün kaydedildi.");
                        dispose();
                        new AdminFashionPage().setVisible(true);
                    } catch (IOException e) {
