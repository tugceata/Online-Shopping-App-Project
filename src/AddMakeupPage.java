...
public class AddMakeupPage extends JFrame { 
    ...
    public AddMakeupPage() {
        ...
        addMakeupPageBtnActionListener back = new addMakeupPageBtnActionListener();
        backBtn.addActionListener(back);
        bottomPanel.add(backBtn);

        addProductBtn = new JButton("Ürünü Kaydet");
        addMakeupPageBtnActionListener add = new addMakeupPageBtnActionListener();
    ...
    private class addMakeupPageBtnActionListener implements ActionListener {
        @Override
        public void actionPerformed(ActionEvent evt) {
            if (evt.getSource() == addProductBtn) {
                ...
                try (BufferedReader br = new BufferedReader(new FileReader("src/makeupproducts.txt"))) {
                    String line;
                ...
                if (!found) {
                    try (FileWriter filewriter = new FileWriter("src/makeupproducts.txt", true)) {
                        maxID++; 
                        String temp = ("Makyaj," + maxID + "," + productNameTf.getText().trim() + ","
                                + productPriceTf.getText().trim() + "," + productStockTf.getText().trim() + "\r\n");
                        filewriter.write(temp);
                        JOptionPane.showMessageDialog(null, "Yeni ürün kaydedildi.");
                        dispose();
                        new AdminMakeupPage().setVisible(true);
                    } catch (IOException e) {
